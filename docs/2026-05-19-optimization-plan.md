...
Phase 5 所有 agent 完成后，进入**测试冲刺阶段**：

1. **并发测试 agents**：为所有新功能补充测试用例
   - 前端: 组件测试、store 测试、页面渲染测试
   - Rust: core 层单元测试、command 层集成测试
2. **并发 Bug fix agents**：全面审查新代码
   - 检查所有新增页面的边界情况（空状态、加载态、错误态）
   - 检查 Rust 错误处理完整性
   - 验证 UI 交互流畅度
3. **最终验证**：全量 build + test + cargo check
```
