...
function decode() {
  ...
  try {
    ...
    // Sort keys by length (longest first) to handle overlapping patterns correctly
    const sortedKeys = Object.keys(reverseEntities).sort(
      (a, b) => b.length - a.length
    );
    for (const entity of sortedKeys) {
      result = result.replace(new RegExp(entity, "g"), reverseEntities[entity]);
    }
    output.value = result;
  } catch (e) {
...
<template>
  <div class="max-w-xl">
    <h3 class="text-lg font-display font-bold text-ink">HTML 实体编解码</h3>
    <p class="mt-1 text-sm text-ink-faint">对 HTML 特殊字符进行实体编码或解码</p>
    <div class="mt-5 space-y-4">
      <div>
        ...
        <textarea
          ...
          placeholder='输入要编码或解码的 HTML 文本，如 <div class="example">'
        />
      </div>
