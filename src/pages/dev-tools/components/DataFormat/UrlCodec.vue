...
function encode() {
  ...
  try {
    if (useFullUri.value) {
      // encodeURI encodes a complete URI but does not encode chars that have special meaning in URIs
...
function decode() {
  ...
  try {
    ...
    error.value = "解码失败: 请输入有效的 URL 编码字符串。 " + (e as Error).message;
    output.value = "";
  }
}
...
<template>
  <div class="max-w-xl">
    ...
    <p class="mt-1 text-sm text-ink-faint">对文本进行 URL 编码或解码</p>
