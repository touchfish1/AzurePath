...
function format() {
  ...
  try {
    ...
    error.value = "无效的 YAML: " + (e as Error).message;
    output.value = "";
  }
}
...
<template>
  <div class="max-w-xl">
    <h3 class="text-lg font-display font-bold text-ink">YAML 格式化</h3>
    <p class="mt-1 text-sm text-ink-faint">格式化和验证 YAML 数据</p>
    <div class="mt-5 space-y-4">
      <div>
        <label class="mb-1.5 block text-xs font-medium text-ink-soft">输入 YAML</label>
        <textarea
          v-model="input"
          rows="8"
