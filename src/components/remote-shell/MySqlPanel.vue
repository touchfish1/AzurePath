...
import { Play, Database, Table, Columns2 } from "lucide-vue-next";
...
const columns = ref<MySqlColumnInfo[]>([]);
const query = ref("");
...
async function loadDatabases() {
  loading.value = true;
  try {
    databases.value = await remoteShellMysqlListDatabases(props.connId);
...
watch(selectedDatabase, async (db) => {
  ...
  try {
    tables.value = await remoteShellMysqlListTables(props.connId, db);
  } catch {
    tables.value = [];
  }
});
...
watch(selectedTable, async (table) => {
  columns.value = [];
  if (!table || !selectedDatabase.value) return;
  try {
    columns.value = await remoteShellMysqlDescribeTable(
      props.connId,
      selectedDatabase.value,
      table
    );
...
async function executeQuery() {
  ...
  queryResult.value = null;
  try {
    queryResult.value = await remoteShellMysqlExecuteQuery(
      props.connId,
      selectedDatabase.value,
      query.value
    );
...
<template>
  <div class="flex h-full flex-col gap-4 overflow-y-auto">
    ...
    <div class="grid grid-cols-[1fr_1fr] gap-4">
      ...
      <div>
        ...
        <div class="rounded-xl border border-paper-deep/20 bg-paper-warm/30">
          ...
          <div
            v-for="t in tables"
            :key="t"
            class="cursor-pointer border-b border-paper-deep/10 px-4 py-2 text-sm transition-colors last:border-b-0 hover:bg-paper-deep/20"
            :class="selectedTable === t ? 'bg-bamboo/10 text-bamboo font-medium' : 'text-ink-soft hover:text-ink'"
      ...
      <div>
        <div class="mb-2 flex items-center gap-1.5">
          <Columns2 class="h-4 w-4 text-ink-faint" />
          <span class="text-xs font-medium text-ink-soft">字段信息</span>
        </div>
          ...
          <table v-if="columns.length > 0" class="w-full text-sm">
            <thead>
              <tr class="bg-paper-deep/10 text-ink-soft text-xs uppercase tracking-wider">
                ...
                <th class="px-3 py-2 text-left">可空</th>
                <th class="px-3 py-2 text-left">键</th>
                <th class="px-3 py-2 text-left">默认值</th>
              </tr>
            </thead>
            <tbody>
              <tr
              ...
              >
                <td class="px-3 py-2 font-mono text-ink">{{ col.field }}</td>
                <td class="px-3 py-2 font-mono text-xs text-ink-soft">{{ col.dbType }}</td>
                <td class="px-3 py-2 text-ink-soft">{{ col.nullable ? "YES" : "NO" }}</td>
                <td class="px-3 py-2 font-mono text-xs text-ink-soft">{{ col.key || "-" }}</td>
                <td class="px-3 py-2 font-mono text-xs text-ink-soft">{{ col.default ?? "NULL" }}</td>
              </tr>
            </tbody>
          </table>
          <div
